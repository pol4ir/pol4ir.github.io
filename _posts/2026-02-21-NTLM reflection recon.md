...
As I conduct further tests, I believe that relying on different error statuses may not be a viable approach since the network traffic is the same. This is because the patch <i>"prevents the exploitation of the vulnerability by removing the ability to coerce machines into authenticating via Kerberos by registering a DNS record with marshalled target information."</i> I also expect the same behavior for NTLM, since coercion does not occur on patched targets.
